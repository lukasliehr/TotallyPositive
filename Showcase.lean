...
/-- Definition of a totally positive function -/
def IsTP (g : ℝ → ℝ) : Prop :=
  g ≠ 0 ∧
  ∀ n : ℕ, ∀ a b : Fin n → ℝ,
...
/-- Definition of a totally positive, integrable, continuous function -/
def IsTPIntegrableContinuous (g : ℝ → ℝ) : Prop :=
  IsTP g ∧ Integrable g ∧ Continuous g

/-- Definition of an element of the Gabor system -/
...
/-- Proof that GaborAtom_Function is an element of L²(ℝ) when g is a totally positive, integrable, continuous function -/
...
/-- The Gabor atom as an actual element of L²(ℝ) -/
def GaborAtom
    (g : ℝ → ℝ) (hg : IsTPIntegrableContinuous g)
    (α β : ℝ) (m n : ℤ) :
...
/-- Property that the Gabor system G(g, α, β) is a frame -/
def IsGaborFrame
